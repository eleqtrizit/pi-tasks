...
export default function (pi: ExtensionAPI) {
  ...
  pi.on("session_start", async (_event, ctx) => {
    ...
    for (const entry of ctx.sessionManager.getEntries()) {
      if (
        entry.type === "custom" &&
        entry.customType === TASK_STATE_ENTRY_TYPE
      ) {
  ...
  pi.registerCommand("list-tasks", {
    ...
    handler: async (_args, _ctx) => {
      try {
        ...
        pi.sendMessage({
          customType: "list-tasks",
          content: `list-tasks failed: ${message}`,
          display: true,
        });
      }
    },
  ...
  pi.registerCommand("clear-tasks", {
    ...
    handler: async (_args, ctx) => {
      try {
        ...
        pi.sendMessage({
          customType: "clear-tasks",
          content: `clear-tasks failed: ${message}`,
          display: true,
        });
      }
    },
  ...
  pi.registerTool({
    ...
    async execute(
      _toolCallId,
      params: Static<typeof TaskCreateParamsSchema>,
      _signal,
      _onUpdate,
      ctx,
    ) {
      if (!Array.isArray((params as { addBlockedBy?: unknown }).addBlockedBy)) {
        const message =
          "Missing required field `addBlockedBy`. Pass `addBlockedBy: []` when the task has no dependencies.";
        return {
  ...
  pi.registerTool({
    ...
    async execute(
      _toolCallId,
      params: Static<typeof TaskUpdateParamsSchema>,
      _signal,
      _onUpdate,
      ctx,
    ) {
      try {
        ...
        return {
          content: [
            {
              type: "text",
              text: `Updated task #${updatedTask.id}: ${updatedTask.subject}`,
            },
          ],
          details: updatedTask,
        };
  ...
  pi.registerTool({
    ...
    async execute(
      _toolCallId,
      params: Static<typeof TaskGetParamsSchema>,
      _signal,
      _onUpdate,
      _ctx,
    ) {
      try {
        ...
        const blockedByText =
          taskItem.blockedBy.length > 0
            ? taskItem.blockedBy.join(", ")
            : "none";
        const blocksText =
          taskItem.blocks.length > 0 ? taskItem.blocks.join(", ") : "none";
        const text = [
  ...
  pi.registerTool({
    ...
    async execute(
      _toolCallId,
      _params: Static<typeof TaskListParamsSchema>,
      _signal,
      _onUpdate,
      _ctx,
    ) {
    ...
    async execute(
      ...
      try {
        ...
        const batch = tasks.filter(
          (t) => t.status === "pending" && !t.isBlocked,
        );
        ...
        return {
          content: [
            { type: "text", text: `get_batch_of_tasks failed: ${message}` },
          ],
          details: { error: message },
        };
      }
