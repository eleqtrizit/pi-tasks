...
async function loadExtensionWithHarness(entries: unknown[] = []) {
    ...
    const extensionFactory = (extensionModule as unknown as { default: (pi: any) => void }).default;
    extensionFactory(pi);
...
describe.sequential('pi taskgraph extension tools', () => {
    ...
    it('returns predictable errors for missing and blocked tasks', async () => {
        await withIsolatedTaskStoreContext(async () => {
            ...
            expect(blockedUpdateResult.content[0]?.text).toContain('task_update failed');
            expect((blockedUpdateResult.details as { error?: string }).error).toContain('blocked');
            expect(missingGetResult.content[0]?.text).toContain('task_get failed');
            expect((missingGetResult.details as { error?: string }).error).toContain('does not exist');
        });
    });
});
