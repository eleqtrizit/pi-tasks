...
function formatBlockedBy(task: TaskListItem): string {
    ...
    const blockedByLabel = task.blockedBy.map((dependencyId) => `#${dependencyId}`).join(', ');
...
export function renderTaskList(taskItems: TaskListItem[]): string {
    ...
    const inProgressCount = taskItems.filter((t) => t.status === 'in_progress').length;
    ...
    const sortedTasks = [...taskItems].sort((a, b) => Number(a.id) - Number(b.id));

    const rows = sortedTasks.map((task) => {
        const symbol = getStatusSymbol(task);
        const status = STATUS_LABELS[task.status] ?? task.status;
        const blocked = task.isBlocked ? ` ⚠ blocked by ${task.blockedBy.map((id) => `#${id}`).join(', ')}` : '';
...
export function renderTaskWidget(taskItems: TaskListItem[]): string[] {
    ...
    const openCount = taskItems.filter((task) => task.status === 'pending').length;
    ...
    const visibleTasks = incompleteTasks.slice(0, MAX_VISIBLE_TASKS).map(formatTaskLine);
