...
import { Task, TaskCreateParams, TaskListItem, TaskStatus, TaskUpdateParams } from './task-model.js';
...
type TaskFileInput = Pick<TaskCreateParams, 'subject'> &
    Partial<Pick<TaskCreateParams, 'description' | 'activeForm' | 'metadata' | 'addBlockedBy'>>;
...
function isDependencyBlocked(tasksById: Map<string, Task>, task: Task): boolean {
    return task.blockedBy.some((dependencyId) => tasksById.get(dependencyId)?.status !== 'completed');
}

export function resolveTaskListId(existingSessionListId: string | undefined): string {
...
export class TaskStore {
    ...
    public async listTasks(): Promise<Task[]> {
        ...
        const tasks = await Promise.all(
            taskFiles.map(async (taskFileName) => {
                const taskContent = await readFile(join(this.listDirectory, taskFileName), 'utf-8');
    ...
    private async getTaskOrNull(taskId: string): Promise<Task | null> {
        ...
        try {
            const taskContent = await readFile(getTaskFilePath(this.listDirectory, taskId), 'utf-8');
            return JSON.parse(taskContent) as Task;
        } catch {
            return null;
        }
    ...
    private async getNextTaskId(): Promise<string> {
        ...
        const maxTaskId = tasks.reduce((maxId, task) => Math.max(maxId, Number(task.id)), 0);
        return String(maxTaskId + 1);
    }

    private assertAllowedStatusChange(tasksById: Map<string, Task>, task: Task, nextStatus: TaskStatus): void {
        if (nextStatus === 'pending') {
            return;
        }

        if (isDependencyBlocked(tasksById, task)) {
            throw new TaskStoreError(`Task #${task.id} is blocked by unfinished dependencies.`);
        }
    }
